...
Market Intelligence Lab v0.6.0 is an authenticated research and simulation application. It must not be used as a brokerage gateway or real-money execution system.
...
- SEC documents and external reports are untrusted. v0.6 stores references and normalized data, not committed raw archives or executable report content.
- Optional libraries load behind adapters. LEAN execution is disabled by default and accepts no live mode or brokerage credentials.
- License-policy CI blocks unknown licenses and restricted-license vendoring; it is not a substitute for legal advice.
