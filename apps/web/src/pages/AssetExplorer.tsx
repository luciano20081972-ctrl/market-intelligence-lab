...
export function AssetExplorer() {
  ...
  return <section>
    ...
    <div className="toolbar"><label className="search-field"><span>Search</span><input aria-label="Search assets" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Symbol or company" /></label><label><span>Status</span><select aria-label="Asset status" value={active} onChange={e => { setActive(e.target.value); setPage(1); }}><option value="true">Active</option><option value="false">Inactive</option><option value="">All</option></select></label><label><span>Sort by</span><select aria-label="Sort assets" value={sort} onChange={e => setSort(e.target.value)}><option value="symbol">Symbol</option><option value="name">Name</option><option value="asset_type">Asset type</option><option value="exchange">Exchange</option></select></label></div>
    {query.isLoading ? <LoadingState label="Loading assets" /> : query.error ? <ErrorState error={query.error} /> : query.data!.items.length === 0 ? <EmptyState title="No matching assets" detail="Try a broader symbol or company name." /> : <div className="table-card"><table><thead><tr><th>Symbol</th><th>Name</th><th>Type</th><th>Exchange</th><th>Capability</th><th>Latest price</th><th>Feed / freshness</th></tr></thead><tbody>{query.data!.items.map(asset => <tr key={asset.id}><td><Link className="symbol" to={`/assets/${asset.symbol}`}>{asset.symbol}</Link></td><td>{asset.name}</td><td><span className="tag">{asset.asset_type}</span></td><td>{asset.exchange}</td><td><span className="tag">{asset.capability.replaceAll("_", " ")}</span></td><td className="number">{asset.latest_price ? `$${Number(asset.latest_price).toFixed(2)}` : "—"}</td><td>{asset.feed} · {asset.freshness}</td></tr>)}</tbody></table>
