...
export const api = {
  ...
  operationsCenter: () => request<{ overall: string; categories: Record<string, string>; open_alerts: number; stale_datasets: number; maintenance: { enabled: boolean; reason: string | null }; latest_backup: { created_at: string; status: string; verification_state: string } | null }>("/api/v1/operations/center"),
  freshnessStatuses: () => request<Array<{ id: string; provider: string; dataset: string; status: string; last_eligible_update_at: string | null; expected_next_update_at: string | null; current_error: string | null; provider_delayed: boolean }>>("/api/v1/operations/freshness"),
  operationalAlerts: () => request<Array<{ id: string; severity: string; category: string; status: string; summary: string; impact: string; unaffected: string; recommended_action: string; occurrence_count: number; last_seen_at: string }>>("/api/v1/operations/alerts"),
  scheduledTasks: () => request<Array<{ id: string; name: string; task_type: string; enabled: boolean; schedule_type: string; provider: string | null; next_due_at: string; last_scheduled_at: string | null }>>("/api/v1/operations/scheduled-tasks"),
