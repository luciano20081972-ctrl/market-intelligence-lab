...
test("authenticated workspace research workflow has no severe console errors", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Import Market Data" }).click();
  ...
  await page.getByRole("link", { name: "Market Research" }).click();
  ...
  await page.getByText("More & administration").click();
  await page.getByRole("link", { name: "System Services" }).click();
  await expect(page.getByRole("heading", { name: "Infrastructure services" })).toBeVisible();
  await page.getByRole("link", { name: "Company Filings & Activity" }).click();
  ...
  await page.getByRole("link", { name: "Strategy Optimization" }).click();
  await page.getByRole("button", { name: "Run deterministic optimization" }).click();
  await expect(page.getByText(/No shorting/)).toBeVisible();
  await page.getByText("More & administration").click();
  await page.getByRole("link", { name: "Data Providers & Integrations" }).click();
  ...
  await page.getByRole("link", { name: "Official Data Sources" }).click();
  ...
  await page.getByRole("link", { name: "Economic Data & Revisions" }).click();
  ...
  await page.getByRole("link", { name: "Energy Data Pilot" }).click();
  await expect(page.getByRole("heading", { name: "Retail electricity prices" })).toBeVisible();
  await page.getByRole("link", { name: "Data History & Provenance" }).click();
  await expect(page.getByRole("heading", { name: "Data manifests" })).toBeVisible();
  await page.getByRole("link", { name: "Economic Relationships" }).click();
  await expect(page.getByRole("heading", { name: "Economic Graph Explorer" })).toBeVisible();
  await page.getByLabel("Reference company").selectOption({ label: "Silica Systems" });
  await expect(page.getByRole("heading", { name: "Silica Systems" })).toBeVisible();
  await page.getByRole("link", { name: "Company Drivers" }).click();
  await expect(page.getByRole("heading", { name: "Company Driver Profile" })).toBeVisible();
  await expect(page.getByText(/not a historically validated factor/)).toBeVisible();
  const companyPicker = page.getByLabel("Reference company");
  await companyPicker.selectOption({ label: "Silica Systems" });
  await expect(page.getByText("technology", { exact: true })).toBeVisible();
  await page.getByRole("link", { name: "Relationship Evidence" }).click();
  await expect(page.getByRole("heading", { name: "Relationship Evidence" })).toBeVisible();
  await page.getByLabel("Reference company").selectOption({ label: "Silica Systems" });
  await expect(page.getByText(/reference:semiconductor/).first()).toBeVisible();
  await page.getByRole("link", { name: "Data Relevance" }).click();
  await expect(page.getByRole("heading", { name: "Data Relevance" })).toBeVisible();
  await expect(page.getByText("PROCESS").first()).toBeVisible();
  await page.getByLabel("Reference company").selectOption({ label: "Meridian Air" });
  await expect(page.getByText("noaa.weather")).toBeVisible();
  await page.getByRole("link", { name: "Company Drivers" }).click();
  await page.getByLabel("Reference company").selectOption({ label: "Harvest Fields Cooperative" });
  await expect(page.getByText("weather environmental", { exact: true })).toBeVisible();
  await page.getByRole("link", { name: "Entity Matching Review" }).click();
