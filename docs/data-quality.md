...
Version 0.4.1 adds a strict provider-envelope boundary before these record checks. A reachable HTML verification page, recognized plaintext error, unknown content type/schema, unsupported delimiter, duplicate column/date, out-of-range or invalid date, non-finite/nonpositive price, inconsistent OHLC, fractional/negative volume, or oversized response never becomes a bar. BOM, ASCII header capitalization, surrounding header whitespace, and CRLF/LF normalization do not change market values. No field is inferred or silently substituted.
