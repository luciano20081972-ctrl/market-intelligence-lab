...
Stooq implements daily historical OHLCV and symbol-only asset metadata. It requires no key, maps confirmed simple U.S. stock and ETF symbols such as `AAPL`, `MSFT`, and `SPY` to lowercase `.us`, uses a fixed HTTPS endpoint, and supports one bounded non-paginated request per symbol. Index and class-share punctuation forms remain unsupported until their mapping is independently confirmed.
